...
(  
    ...
    always @( posedge clk )
        if (rst)   sdaOutDly <=  {SDA_DELAY{1'b1}};
        else       sdaOutDly <=  {sdaOutDly[SDA_DELAY-2:0],sdaOutOri};
    ...
    always @( posedge clk )
        if (rst)   mutexStaDly <=  {MUTEX_DELAY{1'b0}};
        else       mutexStaDly <=  {mutexStaDly[MUTEX_DELAY-2:0],mutexStaOri};
    ...
    always @(*) begin
        ...
        STATE[STOP2]:
            ...
            else if(restart || (rdLen==0)) nSTATE[IDLE]  =1'b1;
            else                           nSTATE[START1]=1'b1;
        
        STATE[ERR_LINE]: nSTATE[IDLE]=1'b1;

        STATE[ERR_NACK]: nSTATE[IDLE]=1'b1;
    ...
    always@(posedge clk) 
        ...
        else if(STATE[START1])  busy <=  1;
    
    always@(posedge clk) 
        if(STATE[IDLE])                          restart <=  0;
        else if(STATE[STOP2] && nSTATE[START1])  restart <=  1;
    
    always@(posedge clk)
        if(STATE!=nSTATE) clkCnt <=  0;
    ...
    always@(posedge clk)
        if(STATE[RD1] && halfBit)
            rdBuf[byteCnt*8-1-bitCnt] <=  sdaIn;
    ...
    always@(posedge clk)
        if(STATE[START2]||STATE[STOP1]||STATE[STOP2])
            ...
            if(byteCnt !=0)      sdaOutOri <=  wrBuf[byteCnt*8-1-bitCnt];
            else if (bitCnt==7)  sdaOutOri <=  restart;
            else                 sdaOutOri <=  addr[6-bitCnt];
        end
        else if(STATE[MACK1]||STATE[MACK2])
             sdaOutOri <=  (byteCnt==rdLen+1);
        else sdaOutOri <=  1;

    always@(posedge clk)
        if(STATE[ERR_LINE]) errLine <=  1;
        else                errLine <=  0;

    always@(posedge clk)
        if(STATE[ERR_NACK]) errNack <=  1;
        else                errNack <=  0;
    
    wire errOrDone=(STATE[STOP2] && halfBit && (restart || (rdLen==0)))
                    ||STATE[ERR_LINE]||STATE[ERR_NACK];
    always@(posedge clk)
        if(errOrDone) 
             done <=  1;
        else done <=  0;
    ...
    always@(posedge clk)
        if(STATE[IDLE] && MUTEX_ENABLE && mutexGet) mutexStaOri <=  1;
        else if(errOrDone)                          mutexStaOri <=  0;
