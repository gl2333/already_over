...
module wave_gen
	...
	// //s_T_clk_num, s_invert_clk_num, s_quat_T_clk_num
	...
	always @(posedge i_clk) begin
		...
		else if(i_enable&&(s_clk_cnt==s_invert_clk_num-s_quat_T_clk_num))
			s_phs_90_n	<=	0	;
		else if(i_enable&&(s_clk_cnt==s_quat_T_clk_num))
			s_phs_90_p	<=	1	;	
		else if(i_enable&&(s_clk_cnt==s_invert_clk_num))
			s_phs_0_p	<=	0	;	
		else if(i_enable&&(s_clk_cnt==s_half_T_clk_num))
			s_phs_0_n	<=	1	;	
		else if(i_enable&&(s_clk_cnt==s_invert_clk_num+s_quat_T_clk_num))
			s_phs_90_p	<=	0	;
		else if(i_enable&&(s_clk_cnt==s_half_T_clk_num+s_quat_T_clk_num))
			s_phs_90_n	<=	1	;	
		else if(i_enable&&(s_clk_cnt==s_half_T_clk_num+s_invert_clk_num))
			s_phs_0_n	<=	0	;	
		else if(i_enable&&(s_clk_cnt==0))
			s_phs_0_p	<=	1	;	
		else begin
			s_phs_0_p	<=	s_phs_0_p	;
			s_phs_0_n   <=	s_phs_0_n   ;
