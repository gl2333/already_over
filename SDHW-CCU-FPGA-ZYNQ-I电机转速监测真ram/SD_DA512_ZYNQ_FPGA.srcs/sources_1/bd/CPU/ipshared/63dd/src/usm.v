...
module usm (
    ...
    assign o_irq_usm_sample_small_pulse= (s_cur_st==ST_MOVING & s_next_st==ST_SAMPLE_SMALL);
	assign s_sample_small              = i_sample_small_pulse && i_sample_limit_en;
	...
	always @(*) begin
		...
		ST_MOVING: begin
			if(s_time_out)
				...
				s_next_st   =   ST_LIMIT	;
			else if(s_sample_small)
				s_next_st   =   ST_SAMPLE_SMALL	;
			else
				s_next_st	=	ST_MOVING	;
			end	
		ST_LIMIT: begin
	...
	assign	s_time_out			        =	s_time_out_cnt>=(750_000);//2s
	assign	s_time_wait			        =	s_time_out_cnt>=(50_000);//0.5s
